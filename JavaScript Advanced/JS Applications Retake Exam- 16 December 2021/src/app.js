...
function updateNav() {
  //for manual logout clear the sessionStorage
  const userData = getUserData();

  // id or class may be different If we select by class map the collection
  if (userData) {
    //     document.querySelector('.user').style.display = 'block';
    //     document.querySelector('.guest').style.display = 'none';
    //     document.querySelector('.user span').textContent = `Welcome, ${userData.email}`;
    userNav.map((x) => (x.style.display = "block"));
    guestNav.map((x) => (x.style.display = "none"));
  } else {
    userNav.map((x) => (x.style.display = "none"));
    guestNav.map((x) => (x.style.display = "block"));
...
function onLogout() {
  logout(); //javascript:void(0) in html href
  updateNav();
  page.redirect("/");
}
